...
import { BriefcaseIcon, UsersIcon, ArrowRightIcon, BuildingOffice2Icon, BookOpenIcon } from '@heroicons/react/24/solid'
import Link from 'next/link'
import type { Student, JobPost } from '@/types/database'
...
const OnboardingWizard = dynamic(() => import('@/components/OnboardingWizard').then((m) => ({ default: m.OnboardingWizard })), {
  ssr: false,
})

export type HomeFeedVariant = 'student' | 'docente'

export function HomeFeedDashboard({ variant }: { variant: HomeFeedVariant }) {
  ...
  const isDocente = variant === 'docente'
  ...
  const handleOnboardingComplete = async () => {
    if (!user) return
    await supabase.from('students').update({ onboarding_completed: true }).eq('id', user.id)
  ...
  const loadMyRequest = async () => {
    ...
    try {
      ...
      if (data) {
        const { data: studentData } = await supabase.from('students').select('course, display_label').eq('id', user.id).single()
        setMyRequest({
          ...data,
  ...
  const loadFeedPosts = async () => {
    ...
    try {
      ...
      const result = await supabase.from('posts').select(baseSelect).order('created_at', { ascending: false }).limit(20)
      ...
      if (result.error) {
        ...
        if (portfolioIds.length > 0) {
          const { data: portfolioItems } = await supabase.from('portfolio_items').select('id, title, images').in('id', portfolioIds)
          const portfolioMap = Object.fromEntries(
            (portfolioItems || []).map((pi: { id: string; title: string; images: string[] }) => [pi.id, pi])
          )
      ...
      if (postsData) {
        ...
        const studentUserIds = [
          ...new Set(
            postsData.filter((p) => p.type === 'collaboration_request' && p.request_from !== 'company').map((p) => p.user_id)
          ),
        ]
        let studentCourses: Record<string, string> = {}
        if (studentUserIds.length > 0) {
          const { data: studentsData } = await supabase.from('students').select('id, course, display_label').in('id', studentUserIds)
        ...
        const likedPostIds =
          postIds.length > 0
            ? new Set(
                (await supabase.from('post_likes').select('post_id').eq('user_id', user.id).in('post_id', postIds)).data?.map(
                  (l) => l.post_id
                ) || []
              )
            : new Set<string>()
  ...
  const showOnboarding = !isDocente && student && !student.onboarding_completed

  return (
    <div className="space-y-4">
      {showOnboarding && <OnboardingWizard onComplete={handleOnboardingComplete} />}
      {isDocente && (
        <Card variant="elevated" className="p-5 border border-amber-100 bg-gradient-to-br from-amber-50/80 to-white">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                <BookOpenIcon className="w-6 h-6 text-amber-600 shrink-0" />
                Area tesi di laurea
              </h3>
              <p className="text-sm text-gray-600 mt-1">
                Proposte, relatore, corelatore e stato delle candidature degli studenti.
              </p>
            </div>
            <Link href="/tesi" className="shrink-0">
      ...
      {!isDocente && student && student.year !== null && student.year >= 2 ? (
        <Card variant="elevated" className="p-5 border border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-gray-900">Pubblica una richiesta di tirocinio o collaborazione</h3>
              <p className="text-sm text-gray-600 mt-0.5">Scopri le opportunità attive e fatti trovare dalle aziende</p>
            </div>
      ...
      ) : !isDocente && student && student.year === 1 ? (
        <Card variant="elevated" className="p-5 border border-amber-100 bg-amber-50/50">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-gray-900">Le richieste di tirocinio sono disponibili dal 2° anno</h3>
              <p className="text-sm text-gray-600 mt-0.5">Al momento puoi esplorare le opportunità e completare il tuo profilo</p>
            </div>
          </div>
        </Card>
      ) : null}
      ...
      {postLoading ? (
        ...
        <div className="space-y-4">
          {posts.map((post) => (
            <PostCard key={post.id} post={post} onUpdate={loadFeedPosts} />
          ))}
        </div>
      )}
    </div>
